package frc.robot.commands;

import frc.robot.subsystems.Lights;
import frc.robot.subsystems.SwerveSubsystem;
import swervelib.SwerveInputStream;
import edu.wpi.first.wpilibj2.command.Command;

import edu.wpi.first.math.MathUtil;
import edu.wpi.first.math.geometry.Pose2d;
import edu.wpi.first.wpilibj.DriverStation;
import java.util.function.DoubleSupplier;

public final class Aim extends Command {
    private static final double HUB_OFFSET_M = 24 * 2.54 / 100.0;
    private static final int    BLUE_TAG      = 26;
    private static final int    RED_TAG       = 10;
    private static final double AIM_TOLERANCE_DEG = 2.0;
    private static final double ROTATION_STICK_THRESHOLD = 0.15; // tune this

    private SwerveSubsystem         swerve;
    private final SwerveInputStream driveInput;
    private final Lights            lights;
    private final DoubleSupplier    rotationX;
    private final DoubleSupplier    rotationY;

    private int     targetTag;
    private Pose2d  hubpose;
    private double  targetAngleDegrees;
    private Command aimcmd;

    public Aim(SwerveSubsystem swerve, SwerveInputStream driveAngle, Lights lights,
               DoubleSupplier rotationX, DoubleSupplier rotationY) {
        addRequirements(swerve);
        this.swerve = swerve;
        this.driveInput = driveAngle;
        this.lights = lights;
        this.rotationX = rotationX;
        this.rotationY = rotationY;
    }

    @Override
    public void initialize() {
        targetTag = swerve.getAlliance() == DriverStation.Alliance.Red ? RED_TAG : BLUE_TAG;
        Pose2d tagPose = swerve.GetTagPose(targetTag);
        boolean red = swerve.getAlliance() == DriverStation.Alliance.Red;
        double offsetX = red ? -HUB_OFFSET_M : HUB_OFFSET_M;
        hubpose = new Pose2d(tagPose.getX() + offsetX, tagPose.getY(), tagPose.getRotation());

        driveInput.aim(hubpose).aimWhile(true);

        aimcmd = swerve.driveFieldOriented(driveInput);
        aimcmd.initialize();

        updateTargetAngle();
    }
    
    private void updateTargetAngle() {
        Pose2d robot = swerve.getPose();
        targetAngleDegrees = Math.toDegrees(
            Math.atan2(hubpose.getY() - robot.getY(), hubpose.getX() - robot.getX()));
    }

    @Override
    public void execute() {
        aimcmd.execute();
        updateTargetAngle();

        double headingError = Math.abs(
            MathUtil.inputModulus(swerve.getPose().getRotation().getDegrees() - targetAngleDegrees, -180, 180));
        double distanceToTag = swerve.getDistanceToTag(targetTag);
        System.out.printf("AimAtHub: Tag=%d Heading=%.1f° Error=%.1f° Dist=%.2fm%n",
                          targetTag, targetAngleDegrees, headingError, distanceToTag);

        if (headingError < AIM_TOLERANCE_DEG) {
            lights.green_on().schedule();
        } else {
            lights.red().schedule();
        }
    }

    @Override
    public void end(boolean interrupted) {
        driveInput.aim(hubpose).aimWhile(false); // hand rotation back to the stick
        if (aimcmd != null) aimcmd.cancel();
        lights.off().schedule();
    }

    @Override
    public boolean isFinished() {
        // Cancel as soon as the rotation joystick is deliberately moved.
        return Math.hypot(rotationX.getAsDouble(), rotationY.getAsDouble()) > ROTATION_STICK_THRESHOLD;
    }
}


